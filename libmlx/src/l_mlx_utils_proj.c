		...
		double gradient)
{
	...
	if (var_point->y < plan->y_min)
	{
		...
		if (l_mlx_is_x_on_plan(proj_point.x, plan) && ref_point->y >= plan->y_min)
		{
	...
	else if (var_point->y > plan->y_max)
	{
		...
		if (l_mlx_is_x_on_plan(proj_point.x, plan) && ref_point->y <= plan->y_max)
		{
			proj_point.color = l_mlx_compute_color(var_point, ref_point, &proj_point);
			proj_point.endian = var_point->endian;
			*var_point = proj_point;
		...
		double gradient)
{
	...
	if (var_point->x < plan->x_min)
	{
		...
		if (l_mlx_is_y_on_plan(proj_point.y, plan) && ref_point->x >= plan->x_min)
		{
			proj_point.color = l_mlx_compute_color(var_point, ref_point, &proj_point);
			proj_point.endian = var_point->endian;
			*var_point = proj_point;
	...
	else if (var_point->x > plan->x_max)
	{
		...
		if (l_mlx_is_y_on_plan(proj_point.y, plan) && ref_point->x <= plan->x_max)
		{
			proj_point.color = l_mlx_compute_color(var_point, ref_point, &proj_point);
			proj_point.endian = var_point->endian;
			*var_point = proj_point;
...
int			check_projection(t_point *var_point, t_point *ref_point, t_plan *plan)
{
	...
	if (!l_mlx_is_x_on_plan(var_point->x, plan) && check_proj_x(var_point, ref_point, plan, gradient))
		return (1);
